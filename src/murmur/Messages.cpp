...
void Server::msgAuthenticate(ServerUser *uSource, MumbleProto::Authenticate &msg) {
	...
	if (!bUsernameMustMatchCertCommonName
		|| (bCertRequired
			&& uSource->qsName
				   == uSource->peerCertificateChain()
						  .first() // The first certificate is the one explicitly issued to the user,
								   // while subsequent certificates are intermediate certificates.
		...
		rtType = MumbleProto::Reject_RejectType_UsernameCertMismatch;
	}
