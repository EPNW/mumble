...
void Server::readParams() {
	...
	bUsernameMustMatchCertCommonName = Meta::mp.bUsernameMustMatchCertCommonName;
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
			QStringList qslAllowedSslClientErrors = qsAllowedSslClientErrors.split(",", Qt::SkipEmptyParts);
#else
			// Qt 5.14 introduced the Qt::SplitBehavior flags deprecating the QString fields
			QStringList qslAllowedSslClientErrors = qsAllowedSslClientErrors.split(",", QString::SkipEmptyParts);
#endif
	...
	bUsernameMustMatchCertCommonName =
		getConf("usernameMustMatchCertCommonName", bUsernameMustMatchCertCommonName).toBool();
...
void Server::newClient() {
	...
	forever {
		...
		if (bMctsIncludeHostCAs) {
			config.addCaCertificates(qlMcts);
		} else {
			config.setCaCertificates(qlMcts);
		}
		...
		switch (e.error()) {
			...
			default:
				if (qlAllowedSslClientErrors.contains(e.error())) {
					...
					log(u, QString("SSL Error (%2): %1").arg(e.errorString()).arg(e.error()));
					ok = false;
				}
		}
	}
